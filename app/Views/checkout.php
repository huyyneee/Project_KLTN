...
<div class="max-w-7xl mx-auto py-6 sm:py-8 md:py-10 px-4 bg-gray-50">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="flex justify-between items-start bg-gray-50 rounded-md p-4">
                        <div>
                            <p class="font-semibold text-gray-800">
                                <?php echo htmlspecialchars($defaultAddress['receiver_name']); ?> | <?php echo htmlspecialchars($maskedPhone); ?>
                            </p>
                            <p class="text-sm text-gray-600"><?php echo htmlspecialchars($fullAddress); ?></p>
                        </div>
                        <a href="#" class="text-green-700 hover:text-green-800 font-medium text-sm">Thay đổi</a>
                    </div>
                <?php else: ?>
            ...
            <section class="bg-white rounded-lg shadow-sm p-6">
                ...
                <div class="flex space-x-4">
                    <label class="flex items-center space-x-3 border border-gray-200 rounded-md p-3 cursor-pointer hover:border-green-600 transition flex-1">
                        <input type="radio" name="payment" checked class="text-green-600 focus:ring-green-600">
                        <img src="assets/images/vnpay.png" alt="VNPay" class="w-6 h-6">
                        <span class="text-gray-800 text-sm">Thanh toán trực tuyến VNPay</span>
                    </label>

                    <label class="flex items-center space-x-3 border border-gray-200 rounded-md p-3 cursor-pointer hover:border-green-600 transition flex-1">
                        <input type="radio" name="payment" class="text-green-600 focus:ring-green-600">
                ...
                <div class="divide-y divide-gray-100">
                    <?php if (!empty($cartItems)): ?>
                        ...
                        <?php if (count($cartItems) > $limit): ?>
                            <button id="toggle-items" class="mt-2 text-sm text-green-700 hover:underline">
        ...
        <aside class="md:col-span-1 bg-white rounded-lg shadow-sm p-6 h-fit self-start">
            ...
            <button class="mt-6 w-full bg-green-700 hover:bg-green-800 text-white font-semibold py-2 rounded-md transition">
                Đặt hàng
            </button>
        </aside>
