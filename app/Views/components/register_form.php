        ...
        <form id="register-form" action="/account/register" method="post" class="space-y-4 w-full text-sm">
            ...
            <div class="flex justify-center items-center space-x-2">
                ...
                <div class="flex items-center">
                    <canvas id="captcha-canvas" class="w-64" width="50" height="32" style="border-radius:6px; background:#1f9a6e; cursor:pointer; display:block; height:32px;"></canvas>
                </div>
            </div>
...
(function(){
    ...
    function drawCaptcha(text) {
        ...
        var w = 60, h = 32; // giống ô lấy mã (w-64, height 32px)
    ...
    if (sendBtn) sendBtn.addEventListener('click', function(){
        ...
        }).then(function(res){ return res.json(); }).then(function(js){
            if (js.exists) {
    ...
    try { var v = document.getElementById('verification_code'); if (v) { v.focus(); v.scrollIntoView({behavior:'smooth', block:'center'}); } } catch(e){}
    <?php endif; ?>

    <?php if (!empty($show_modelok)): ?>
    try { showModelOk(<?php echo json_encode($modelok_message ?? ''); ?>); } catch(e){}
    <?php endif; ?>
        ...
        if (!verification || verification === '') {
            ...
            fetch('/account/check-email', {
                ...
                if (json.ok) {
                    ...
                    var v = document.getElementById('verification_code'); if (v) { v.focus(); }
                } else {
